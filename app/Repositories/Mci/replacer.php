...
$search = <<<EOT
    public function getQualityAnalytics(string \$groupBy = 'cabang', string \$cabang = ''): array
    {
        ...
        \$cacheKey = "financing:quality_analytics:g3:{\$groupBy}:".(\$cabang ?: 'all');
        ...
        \$data = Cache::remember(\$cacheKey, 60, function () use (\$groupBy, \$cabang): array {
            ...
            \$akadRows = DB::connection(\$this->connection)->select("
                ...
                FROM TOFLMB a LEFT JOIN SETUPLOAN p ON a.kdprd = p.kdprd
                WHERE a.stsrec = 'A' AND a.stsacc <> 'W' {\$cabangFilter}
            ...
            \$branchCompareRows = DB::connection(\$this->connection)->select("
                ...
                FROM TOFLMB a JOIN CABANG c ON a.kdloc = c.kdloc
            ...
            \$alertRows = DB::connection(\$this->connection)->select("
                ...
                FROM TOFLMB a LEFT JOIN SETUPLOAN p ON a.kdprd = p.kdprd
                WHERE a.stsrec = 'A' AND a.stsacc <> 'W' AND a.colbaru IN ('2','3','4','5') {\$cabangFilter}
                ORDER BY a.osmdlc DESC, a.haritgk DESC
            ", \$bindings);

            // 6. NPF Trend bulanan — month mapped ke nama Indonesia
            \$tahun = date('Y');
            \$currentMonthResult = DB::connection(\$this->connection)->select("SELECT SUBSTRING(MAX(tgl), 3, 2) AS current_month FROM TANGGAL");
            \$currentMonthNum = isset(\$currentMonthResult[0]->current_month) ? (int)\$currentMonthResult[0]->current_month : (int)date('m');
            \$currentMonthStr = str_pad((string)\$currentMonthNum, 2, '0', STR_PAD_LEFT);
            ...
            \$trendRows = DB::connection(\$this->connection)->select("
                ...
                FROM TOFLMBEOM a
                WHERE LEFT(periode, 4) = '{\$tahun}' AND RIGHT(periode, 2) <= '{\$currentMonthStr}'
                  AND stsrec = 'A' AND stsacc <> 'W' {\$cabangFilter}
            ...
            \$eclRows = DB::connection(\$this->connection)->select("
                ...
                FROM TOFLMB a WHERE a.stsrec = 'A' AND a.stsacc <> 'W' {\$cabangFilter}
            ", \$bindings);
            ...
            \$aoMatrixRows = DB::connection(\$this->connection)->select("
                SELECT ISNULL(b.nmao, '(Tanpa AO)') as nama_ao,
                    ...
                    ELSE 0 END as npf_ratio
                FROM TOFLMB a LEFT JOIN AO b ON a.kdaoh = b.kdao
                WHERE a.stsrec = 'A' AND a.stsacc <> 'W' {\$cabangFilter}
            ...
            \$restruTotal = DB::connection(\$this->connection)->select("
                ...
                FROM TOFLMBHP a INNER JOIN TOFLMB b ON a.nokontrak = b.nokontrak
                WHERE b.stsrec = 'A' AND b.stsacc <> 'W' {\$cabangFilter}
            ", \$bindings);
            ...
            \$restruFail = DB::connection(\$this->connection)->select("
                ...
                WHERE b.stsrec = 'A' AND b.stsacc <> 'W'
                  AND b.colbaru IN ('3','4','5') AND CAST(a.colnew AS VARCHAR) IN ('1','2') {\$cabangFilter}
            ", \$bindings);
            ...
            \$totalPPAP = DB::connection(\$this->connection)->table('TOFLMB')
                ...
                ->when(\$cabang !== '', fn (\$q) => \$q->where('kdloc', \$cabang))
                ->sum('ppap');
...
if (strpos($content, $search) !== false) {
    ...
    file_put_contents('c:\laragon\www\dashboard\monitoring-dashboard\app\Repositories\Mci\FinancingRepository.php', $content);
