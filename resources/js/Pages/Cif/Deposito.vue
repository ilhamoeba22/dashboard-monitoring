...
const doExport = async () => {
  ...
  await exportToExcel(store.auditData, `Deposito_${suffix}`)
}
...
const doExportPdf = async () => {
  try {
    ...
    await exportToPdf(store.auditData, currentHeaders.value, 'Pengecekan CIF Deposito', suffix)
  } catch (e) {
...
<template>
  <div class="cif-page px-4 pt-0">
    <div class="cif-hero mb-6">
      ...
      <div class="cif-hero__inner">
        <div class="cif-hero__top">
          <div class="cif-hero__icon" style="background: linear-gradient(135deg, #f59e0b, #b45309); box-shadow: 0 8px 20px rgba(245, 158, 11, 0.4);">
            <v-icon icon="ri-safe-2-line" size="26" color="white" />
          </div>
          <div class="cif-hero__meta">
            <h1 class="cif-hero__title">Pengecekan CIF Deposito</h1>
            <p class="cif-hero__subtitle">Audit kelengkapan dan anomali data nasabah deposito.</p>
    ...
    <div class="fin-filter-card d-flex justify-space-between align-center mb-6">
      ...
      <div class="d-flex align-end">
        <div class="d-flex gap-2">
          <v-btn variant="outlined" color="#64748b" height="40" prepend-icon="ri-refresh-line" @click="store.resetFilters(); store.fetchDeposito()">Reset</v-btn>
    ...
    <v-alert
      ...
      density="comfortable"
    >
      Tidak ada data CIF deposito untuk filter ini.
    </v-alert>
